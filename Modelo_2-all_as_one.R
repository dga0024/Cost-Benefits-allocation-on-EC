...
Costes_all_as_one <- function(N, Consumos_base, Distrib, Num_perfiles_gen, PERFILGEN) {
  ...
  E_producida = Perfiles_gen[,PERFILGEN]
  E_consumida = rowSums(Agentes)
  
  sum(E_producida)

  Horas_NP = which(E_producida > E_consumida)
  
  Balance_horario = as.data.frame(E_consumida - E_producida)
  sum(Balance_horario)
  sum(Agentes_Balance)

  
  Coste_horario = Balance_horario*0
  Coste_horario[-Horas_NP] = Balance_horario[-Horas_NP] * p_grid_buy[-Horas_NP]
  Coste_horario[Horas_NP,1] = Balance_horario[Horas_NP,1] * p_grid_sell[Horas_NP]
  Coste_horario_TOTALES1 = sum(Coste_horario)

  Cost_tras_reparto = Agentes*0
  
  Cost_tras_reparto[Horas_NP,] = (Coef_reparto_Matrix[Horas_NP,] * p_grid_sell[Horas_NP]) * Balance_horario[Horas_NP,1]
  
  Ratio_consumo_agentes = Agentes/E_consumida
  Cost_tras_reparto[-Horas_NP,] = (Ratio_consumo_agentes[-Horas_NP,] * p_grid_buy[-Horas_NP]) * Balance_horario[-Horas_NP,1]
  sum(Cost_tras_reparto)
  
  #SURPLUS
  Surplus = Cost_NoTrade - Cost_tras_reparto
  
  Surplus1 = Cost_NoTrade_TOTAL1 - Coste_horario_TOTALES1
  Surplus2 = sum((p_grid_buy-p_grid_sell)*E_TRANSFER)
  
  Surplus3 = sum(Surplus)
  Surplus_ratio = Surplus3/Coste_horario_TOTALES1*100
  ...
  Surplus_NP2 = sum(Cost_NoTrade[ID_NP]) - sum(Cost_tras_reparto[ID_NP])
  Surplus_NC2 = sum(Cost_NoTrade[ID_NC]) - sum(Cost_tras_reparto[ID_NC])
  ...
  RESUMEN = data.frame(Tipo_Agente = colnames(Consumos_base)[Distrib],
                       ...
                       Cost_Trade = colSums(Cost_tras_reparto),
                       Surplus = as.numeric(colSums(Cost_NoTrade) - colSums(Cost_tras_reparto)),
                       Surplus_ratiox100_FV = ((colSums(Cost_original) - colSums(Cost_tras_reparto))/colSums(Cost_original))*100,
                       Surplus_ratiox100_ALLOC = ((colSums(Cost_NoTrade) - colSums(Cost_tras_reparto))/colSums(Cost_original))*100,
                       Part_beneficiosa = (colSums(Cost_NoTrade) > colSums(Cost_tras_reparto)),
  ...
  titulo = paste0("ALL-AS-ONE  Gen ",colnames(Perfiles_gen[PERFILGEN]),"x100 N=",N," ",p_tr_case)
  ...
  GraficoPLOT1 = 
    ...
    geom_line(data = RESUMEN2,aes(x=seq(1,N,1), y=Cost_reparto,colour = "Cost_Reparto" ),
    ...
    scale_color_manual(values = c("Cost_NoTrade" = "red", "Cost_Reparto" = "blue"))+
