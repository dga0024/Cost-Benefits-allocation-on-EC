...
Costes_bill_sharing <- function(N, Consumos_base, Distrib, Num_perfiles_gen, PERFILGEN) {
  ...
  Cost_tras_reparto[Horas_NP,] = TempNP_v2[Horas_NP,] * (Coste_horario[Horas_NP,1])
  Cost_tras_reparto[-Horas_NP,] = TempNC_v2[-Horas_NP,] * (Coste_horario[-Horas_NP,1]) 
  ...
  titulo = paste0("BILL-SHARING  Gen ",colnames(Perfiles_gen[PERFILGEN]),"x100 N=",N," ",p_tr_case)
  ...
  GraficoPLOT1 = 
    ...
    geom_line(data = RESUMEN2,aes(x=seq(1,N,1), y=Cost_Trade,colour = "Cost_Reparto" ),
